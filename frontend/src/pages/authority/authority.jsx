...
const Authority = () => {
  ...
  // Personnel management state (no busy tracking)
  const [personnel] = useState([
    { id: 1, name: "মোঃ রফিকুল ইসলাম", role: "সিনিয়র পরিদর্শক"},
    { id: 2, name: "নাসরিন আক্তার", role: "জেলা কর্মকর্তা"},
    { id: 3, name: "কামাল হোসেন", role: "তদন্ত কর্মকর্তা"},
    { id: 4, name: "সালমা বেগম", role: "সহকারী পরিচালক" },
    { id: 5, name: "মোঃ আলী হাসান", role: "মাঠকর্মী"},
    { id: 6, name: "রেহানা পারভীন", role: "জনসংযোগ কর্মকর্তা"},
  ]);
  ...
  // Assign personnel to complaint (no busy tracking)
  const handleAssign = async (complaintId, personId) => {
    if (!personId) {
      alert("অনুগ্রহ করে একজন কর্মী নির্বাচন করুন");
      return;
    }
    
    const person = personnel.find(p => p.id === parseInt(personId));
    
    try {
      const response = await fetch(`http://localhost:5000/complaints/${complaintId}/assign`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          assignedTo: {
      ...
      if (response.ok) {
        ...
        alert('কর্মী বরাদ্দ করা হয়েছে!');
      } else {
        alert('কর্মী বরাদ্দ করতে ব্যর্থ হয়েছে।');
      }
    } catch (error) {
  ...
  const handleUnassign = async (complaintId) => {
    try {
      const response = await fetch(`http://localhost:5000/complaints/${complaintId}/unassign`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (response.ok) {
        fetchComplaints(); // Refresh complaints list
        alert('বরাদ্দ বাতিল করা হয়েছে!');
      } else {
        alert('বরাদ্দ বাতিল করতে ব্যর্থ হয়েছে।');
      }
    } catch (error) {
      ...
      <div className="container-fluid">
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            ...
            <p className="cm-subtitle">সকল অভিযোগ দেখুন, স্ট্যাটাস আপডেট করুন এবং কর্মী বরাদ্দ করুন</p>
          </div>
              ...
              <tbody>
                {filteredComplaints.length === 0 ? (
                  ...
                  filteredComplaints.map((complaint, index) => {
                    const isAssigned = complaint.assignedTo && complaint.assignedTo.id;
                    const assignedPerson = isAssigned ? complaint.assignedTo : null;
                    ...
                    return (
                      <tr key={complaint._id}>
                        ...
                        <td className="assign-cell">
                          {isAssigned && assignedPerson ? (
                            <div className="assigned-person-info">
                              <div className="person-avatar-mini">{assignedPerson.initials}</div>
                              <div className="person-details">
                                <div className="person-name">{assignedPerson.name}</div>
                                <div className="person-role">{assignedPerson.role}</div>
                              </div>
                              <button 
                                className="btn-unassign-mini"
                            ...
                            <div className="assign-select-wrapper">
                              <select 
                                ...
                                value={selections[complaint._id] || ""}
                              ...
                              <button 
                                ...
                                onClick={() => handleAssign(complaint._id, selections[complaint._id])}
                                disabled={!selections[complaint._id]}
                              >
                                বরাদ্দ
                              </button>
                            </div>
